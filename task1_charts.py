...
# --- Chart 1: Total sales by LIFESTAGE x PREMIUM_CUSTOMER (heatmap-style) ---
...
# --- Chart 2: Average spend per customer vs number of customers (bubble) ---
fig, ax = plt.subplots(figsize=(9, 6))
colors = {"Budget": "#f97316", "Mainstream": "#2563eb", "Premium": "#16a34a"}
for pc, grp in segment_summary.groupby("PREMIUM_CUSTOMER"):
    ax.scatter(grp["N_CUSTOMERS"], grp["AVG_SPEND_PER_CUSTOMER"],
               s=grp["TOT_SALES"] / 400, alpha=0.7, color=colors[pc], label=pc, edgecolors="white")
    for _, row in grp.iterrows():
        ax.annotate(row["LIFESTAGE"].title(), (row["N_CUSTOMERS"], row["AVG_SPEND_PER_CUSTOMER"]),
...
# --- Chart 3: Brand affinity for the target segment (Mainstream Young Singles/Couples) ---
...
bars = ax.barh(affinity.index[::-1], affinity.values[::-1], color="#2563eb")
...
# --- Chart 4: Sales trend over the year ---
